...
import ntsecuritycon  # pip install pywin32
...
def get_permissions(file_path):
    """Retrieves owner and ACL using native Windows APIs via pywin32."""
    owner_name = "UNKNOWN"
    acl_summary = []
    mode = "N/A"

    try:
        # 1. Get Basic Octal Mode
        st = os.stat(file_path)
        mode = oct(st.st_mode & 0o777)

        # 2. Get Security Descriptor (Owner and DACL)
        sd = win32security.GetFileSecurity(
            file_path, win32security.OWNER_SECURITY_INFORMATION | win32security.DACL_SECURITY_INFORMATION
        )
        ...
        # 4. Parse DACL (Permissions for all users)
        dacl = sd.GetSecurityDescriptorDacl()
        if dacl:
            for i in range(dacl.GetAceCount()):
                rev, access, sid = dacl.GetAce(i)
                try:
                    user_name, user_domain, user_type = win32security.LookupAccountSid(None, sid)
                    
                    # Identify high-level access rights
                    rights = []
                    if (access & ntsecuritycon.FILE_ALL_ACCESS) == ntsecuritycon.FILE_ALL_ACCESS:
                        rights.append("Full Control")
                    else:
                        if (access & ntsecuritycon.FILE_GENERIC_READ) == ntsecuritycon.FILE_GENERIC_READ:
                            rights.append("Read")
                        if (access & ntsecuritycon.FILE_GENERIC_WRITE) == ntsecuritycon.FILE_GENERIC_WRITE:
                            rights.append("Write")
                        if (access & ntsecuritycon.FILE_GENERIC_EXECUTE) == ntsecuritycon.FILE_GENERIC_EXECUTE:
                            rights.append("Execute")
                    
                    perm_str = ", ".join(rights) if rights else "Special"
                    acl_summary.append(f"{user_domain}\\{user_name} ({perm_str})")
                except:
                    continue

    except Exception as e:
        owner_name = f"ACCESS_DENIED/ERROR"

    return {
        "Owner": owner_name, 
        "Mode": mode, 
        "User_Permissions": " ; ".join(acl_summary) if acl_summary else "N/A"
    }
...
def scan_file(file_path, force_filename_only=False):
    """Performs forensic analysis and captures metadata for matches."""
    try:
        ...
        if matches_found:
            ...
            return {
                ...
                "Owner": perms["Owner"],
                "User Permissions (ACL)": perms["User_Permissions"],
                "Mode": perms["Mode"],
                "Full Path": file_path,
