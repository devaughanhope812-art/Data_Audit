...
def scan_file(file_path, force_filename_only=False):
    try:
        ...
        found_hits = {}
        hit_details = []
        total_sample_hits = 0 # TRACKS SCALE FOR MAGNITUDE SAMPLING
        ...
        if not force_filename_only and ext in VALID_EXTENSIONS:
            ...
            if f_size <= size_limit:
                text_to_scan += " " + extract_content_sample(file_path, ext)

        # MAGNITUDE SAMPLING LOGIC: COUNT ALL MATCHES IN SAMPLE
        for category, patterns in SENSITIVE_PATTERNS.items():
            for pattern in patterns:
                matches = pattern.findall(text_to_scan)
                if matches:
                    total_sample_hits += len(matches)
        ...
        if "Credentials" in found_hits: final_reasons.append(f"Credentials ({found_hits['Credentials']})")
        if "Identity" in found_hits: final_reasons.append(f"Identity ({found_hits['Identity']})")
        if "Financial" in found_hits: final_reasons.append(f"Financial ({found_hits['Financial']})")
        if "Biometric" in found_hits: final_reasons.append(f"Biometric ({found_hits['Biometric']})")

        if final_reasons:
            meta = get_forensic_metadata(file_path)
            stats = os.stat(file_path)
            
            # CALCULATE DENSITY (HITS / KB OF SCANNED SAMPLE)
            sample_kb = len(text_to_scan.encode(errors='ignore')) / 1024
            density = round(total_sample_hits / sample_kb, 2) if sample_kb > 0 else 0

            return {
                "File Name": file_name,
                "Patterns Found": " | ".join(final_reasons),
                "Hit_Density_per_KB": density, # MAGNITUDE METRIC
                "Owner": meta["Owner"],
